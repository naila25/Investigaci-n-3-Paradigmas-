...
function crearTablaUsuarios(usuarios) {
  ...
  tabla.innerHTML = `
    <thead class="bg-gradient-to-r from-blue-700 to-blue-700 text-white">
      <tr>
  ...
  usuarios.forEach((usuario, index) => {
    ...
    fila.innerHTML = `
      ...
      <td class="py-3 px-4 text-center">
        <div class="flex flex-col gap-2">
          <button data-index="${index}" class="btnVerHistorial bg-blue-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600 transition font-semibold">
            📖 Ver Historial
          </button>
          <button data-index="${index}" class="btnEliminarUsuario bg-red-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-600 transition font-semibold">
            🗑️ Eliminar
          </button>
        </div>
      </td>
  ...
  // Escuchar clics en botones de eliminar usuario
  document.querySelectorAll(".btnEliminarUsuario").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (confirm(`¿Estás seguro de eliminar al usuario "${usuario.nombre}"?`)) {
        const resultado = biblioteca.eliminarUsuario(index);
